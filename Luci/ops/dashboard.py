...
One stdlib HTTP server reading the files Luci already writes:
...
def _get_agent():
    ...
    if _agent is None:
        ...
        _agent = Luci(settings=settings, conn=conn)
    return _agent
...
# row assumes the default model (see PROVIDERS) — real cost varies with LUCI_MODEL.
PRICING = {
...
def collect() -> dict:
    ...
    from Luci.memory import REPO_SKILLS
    ...
    return {
        ...
        "skills": skills,
        "applications": rows(
            "SELECT company, role, status, notes, applied_at, updated_at FROM applications "
            "ORDER BY updated_at DESC"),
        "learnings": rows(
            "SELECT topic, content, tags, source, learned_at FROM learnings ORDER BY learned_at DESC"),
        "outreach": rows(
            "SELECT company, contact, draft, context, created_at FROM outreach_drafts ORDER BY created_at DESC"),
...
def _rel_to_home(path, home) -> str:
    """Path relative to LUCI_HOME if it lives there, else the repo-relative
...
def session_list(conn) -> list[dict]:
    ...
    for g in groups:
        ...
        preview = ""
        if last:
            preview = ("you: " if last["role"] == "user" else "luci: ") + last["content"][:80]
        out.append({"id": sid,
...
_LUCI = {"log_learning", "search_learnings", "track_application", "list_applications", "draft_outreach"}
...
def tools_info() -> dict:
    ...
    if _agent is not None:
        ...
        from Luci.tools import applications, calendar, learning, memory_admin, messages, notes, outreach, search
        ...
        tools = [calendar.make_tool(
                 ...
                 search.make_tool(),
                 learning.make_log_tool(conn), learning.make_search_tool(conn),
                 applications.make_track_tool(conn), applications.make_list_tool(conn),
                 outreach.make_tool(conn),
    ...
    from Luci.tools.experimental import PLANNED

    from Luci.tools.google_calendar import is_available as google_deps_installed
    ...
    return {"catalog": catalog, "mcp": mcp, "apple_on": settings.apple_tools,
            ...
            "google_calendar_on": settings.google_calendar,
            "google_calendar_ready": google_deps_installed(),
            "google_credentials_found": settings.google_calendar_credentials.exists()}
...
def transcribe_audio(raw: bytes) -> dict:
    ...
    with _whisper_lock:
        if _whisper is None:
            _whisper = WhisperModel(os.getenv("LUCI_WHISPER_MODEL", "base"), compute_type="int8")
...
def _editor_cmd() -> list[str] | None:
    """The user's code editor CLI: $LUCI_EDITOR, then cursor, then code."""
    import shutil

    custom = os.getenv("LUCI_EDITOR")
...
def reveal_path(rel: str) -> dict:
    """Open a file/folder under LUCI_HOME — in the user's code editor if one
    is on PATH (cursor/code/$LUCI_EDITOR), otherwise reveal in Finder.
    Restricted to paths inside LUCI_HOME."""
    import subprocess
    import sys
    ...
    if target != home and home not in target.parents:
        return {"error": "path is outside the .luci home"}
...
def memory_action(payload: dict) -> dict:
    ...
    if action == "save_skill":
        ...
        from Luci.memory import REPO_SKILLS
        from Luci.memory.procedural.loader import _parse_text
...
def settings_info() -> dict:
    ...
    the full key."""
    from Luci.loop.models import PROVIDERS
    ...
    return {
        ...
        # calendar sync toggles — booleans, not secrets, but same live-editable pattern
        "apple_calendar": s.apple_calendar,
        "apple_tools": s.apple_tools,
        "google_calendar": s.google_calendar,
        "google_credentials_path": str(s.google_calendar_credentials),
        "google_credentials_found": s.google_calendar_credentials.exists(),
    }
...
def apply_settings(payload: dict) -> dict:
    ...
    writable = ({"LUCI_PROVIDER", "LUCI_MODEL", "LUCI_SMALL_MODEL", "TAVILY_API_KEY",
                 "LUCI_APPLE_CALENDAR", "LUCI_APPLE_TOOLS", "LUCI_GOOGLE_CALENDAR", "LUCI_GOOGLE_CREDENTIALS"}
                | {p.key_env for p in PROVIDERS.values()})
    env_path = find_dotenv(usecwd=True) or ".env"

    updates = {"LUCI_PROVIDER": provider,
               "LUCI_MODEL": payload.get("model", "") or "",
               "LUCI_SMALL_MODEL": payload.get("small_model", "") or ""}
    for k, v in (payload.get("keys") or {}).items():
        if k in writable and v:  # only non-empty keys overwrite
            updates[k] = v
    # toggles are booleans, not secrets — "" (unset) is a valid, intentional value
    for k, v in (payload.get("toggles") or {}).items():
        if k in writable:
            updates[k] = v
    ...
    with _agent_lock:
        ...
        try:
            ...
            _agent = Luci(settings=new_settings, conn=conn)
...
def main() -> None:
    # Port precedence: LUCI_DASHBOARD_PORT, then the conventional PORT (used by
    # deploy platforms and IDE preview panes), then 7777. If it's taken, walk on.
    base = int(os.getenv("LUCI_DASHBOARD_PORT") or os.getenv("PORT") or PORT)
    for port in range(base, base + 10):  # walk past a busy port instead of crashing
        ...
        # too (background thread) so you don't need a separate `luci telegram`.
        try:
            from Luci.gateway.telegram import start_in_background
        ...
        print(f"Luci dashboard → http://localhost:{port}  (Ctrl-C to stop)")
