"""
Apple ecosystem tools so luci can brief you on your real Week- 
reading your actual calender.app and mail , and writing reminders / Notes
Opt- IN via LUCI_APPLE_TOOL = 1 ; first use triggers the system Automation 
permission prompts . All AppleScript runs with a timeout and returns 
honest error text so a slow/ denied call never hangs a turn 
"""
...
def read_apple_calendar(days_ahead: int = 7) -> str:
    ...
    with LUCI_APPLE_CALENDARS=Work,Home (enumeration is slow on many calendars)."""
    def go() -> str:
        cals = os.getenv("LUCI_APPLE_CALENDARS", "").strip()
        cal_clause = ""
        if cals:
