...
Pick with LUCI_PROVIDER=anthropic|openai|gemini|deepseek|kimi|glm|openrouter|minimax and
set that provider's API key in .env. Override the model ids with LUCI_MODEL /
LUCI_SMALL_MODEL if the defaults below age out — they're just strings. This
matters most for openrouter: it's a single key in front of hundreds of
models, so LUCI_MODEL=<vendor>/<model> (e.g. "google/gemini-3.5-flash") picks
...
def get_client(settings: Settings):
    ...
    if provider is None:
        raise SystemExit(f"Unknown LUCI_PROVIDER '{settings.provider}'. "
    ...
    timeout = float(os.getenv("LUCI_LLM_TIMEOUT", "120"))
