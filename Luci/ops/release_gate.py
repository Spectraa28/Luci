...
Changed the prompt ? swapped the model? tuned retrieval top-k? run the gate:
        ...
        python -m luci.ops.release_gate (or make gate)
        
Deterministic evals must pass 100% -   they are unit teests; one failure blocks.
judge evals runs when a key is present  and report scores . Exit code 0 = ship.    
"""
...
def report(deterministic: str, judge:str,suites:dict| None = None) -> None:
    """Persistent the latest verdict AND append it to the run history. """
