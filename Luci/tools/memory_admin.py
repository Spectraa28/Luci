"""
Tools that let the agent manage its own memory - so it feels like a 
personal assistant that lerns, not a black box , three tools  :
 manage_memory -- search/ update / delete facts and episodes (CRUD)
 update_soul --  append a durable behaviour rule to soul.md 
 create_skill -- write a new SKILL.md , so the agent build its own procedures 
 
Everything write to the same local files the dashboard shows ; nothing leaves the machine 
"""
...
def make_update_soul_tool(settings) -> Tool:
    from luci.runtime.session import load_soul
