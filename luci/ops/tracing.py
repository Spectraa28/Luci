...
1. JSONL, always on : every turn appends readable lines to .luci/traces/<date>.jsonl.
    a trace is just "what happened , in order" - open the file and read your agents mind . 
    zero dependencies
    
2. OpenTelemetry spans , when OTEL_EXPORTER_OTLP_ENDPOINT is set: the same
   events as a span tree any OTel backend can render . for local dashboard:
   
    pip install 'luci[tracing]'
    phoenix serve
    OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4317 python -m luci
"""
...
class Tracer:
        ...
        if self._otel_tracer and self._span_ctx is not None:
            with self._otel_tracer.start_as_current_span(
                ...
                attributes={
                    ...
                    **{f"luci.{k}": json.dumps(v, default=str) for k, v in event.items()},
                },
            ):
                pass
    ...
    def turn(self, user_message: str):
        ...
        if self._otel_tracer:
            with self._otel_tracer.start_as_current_span(
                "agent_run",
                attributes={"openinference.span.kind": "AGENT", "luci.user_message": user_message},
            ) as span:
                self._span_ctx = span
                try:
                    yield self
