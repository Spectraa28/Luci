...
push-to-talk MVP : press Enter , speak , press ENTER again.
through the exact same loop / memory / eval pipelin as typed text - a gateway 

only moves words in and out 

    ears faster-whisper (local whisper , ~74 MB model downloads on first run)
"""
...
from Luci.gateway.cli import _observer

SAMPLE_RATE = 16000

def record_until_enter():
    """Capture mic audio between two enter presses ; returns a float 32 array"""
    import numpy as np
...
class Ears:
    def __init__(self, model_size: str | None = None):
        ...
        self.model = WhisperModel(
            model_size or os.getenv("LUCI_WHISPER_MODEL", "base"),
            compute_type="int8",
        )

    def transcribe(self, audio, language: str | None = None) -> str:
        segments, _ = self.model.transcribe(
            audio, language=language or os.getenv("LUCI_WHISPER_LANG")
        )
...
class Mouth:
    ...
    def __init__(self):
        self.engine = os.getenv("LUCI_TTS", "").strip().lower()
        self.voice = os.getenv("LUCI_VOICE", "")
        if not self.engine:
    ...
    def speak(self, text: str) -> None:
        ...
        if self.engine == "kokoro":
            ...
            print("(no TTS engine on this platform — set LUCI_TTS=kokoro)")


def matches_wake(text: str, wake_word: str) -> bool:
    """Does a transcript contain the (customizable) wake word?

    Fuzzy on purpose: Whisper hears "luci luci" as "luciluci", "Luci, luci!",
    "walku luci" — or transcribes it as Japanese kana わくわく (the first live
    test!). So: `wake_word` accepts comma-separated variants across scripts
    ("luci luci,わくわく"), normalization keeps kana AND CJK, and matching is
...
def _mic_threshold() -> float:
    ...
    LUCI_MIC_THRESHOLD (lower if it never hears you, higher if it
    wakes on room noise)."""
    return float(os.getenv("LUCI_MIC_THRESHOLD", "0.005"))
...
def wake_loop(luci: luci, mouth: "Mouth", wake_word: str) -> None:
    ...
    - The mic buffer is drained after Luci speaks, so it doesn't wake on
    ...
    ack = os.getenv("LUCI_WAKE_ACK", "Yes?")
    followup = float(os.getenv("LUCI_FOLLOWUP_SECONDS", "8"))  # stay open, Siri-style
    block = SAMPLE_RATE // 10
    # Pin the scout's transcription language to match the wake word's script —
    # otherwise Whisper hears "luci luci" and helpfully writes わくわく, which
    # a latin wake word never matches. Commands after wake still auto-detect.
    wake_lang = os.getenv("LUCI_WAKE_LANG") or ("en" if wake_word.isascii() else None)
        ...
        while True:
            ...
            if not matches_wake(heard_scan, wake_word):
                ...
                if heard_scan:  # near-misses belong in the trace (wake tuning!)
                    luci.tracer.event("wake_scan", {"heard": heard_scan, "matched": False})
                continue
            ...
            # for a follow-up for `followup` seconds — no need to say "luci luci"
            # again (like Siri). A quiet stretch drops back to wake-word mode.
            while True:
                heard = ears.transcribe(record_command(stream))
                if heard:
                    print(f"you › {heard}")
                    result = luci.respond(heard, observer=_observer, source="voice")
...
def main() -> None:
    ...
    luci = luci()
    luci.session.session_id = "voice"   # its own conversation thread in the inbox
    mouth = Mouth()

    # Hands-free by default: always-listening for "luci luci". The default packs
    # in the ways the tiny scanner mis-hears it (luciluci / waka waka / kana), so
    # it triggers reliably. Set LUCI_WAKE_WORD="" for push-to-talk instead.
    wake_word = os.getenv(
        "LUCI_WAKE_WORD", "luci luci,luciluci,luci,waka waka,wako wako,walk walk,わくわく"
    ).strip()
    if wake_word:
        try:
            wake_loop(luci, mouth, wake_word)
    ...
    ears = Ears()
    print("Voice Luci ready. Press Enter to talk, Ctrl-C to quit.")
    while True:
        try:
            input("\npress Enter to talk… ")
