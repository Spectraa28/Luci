...
import _sqlite3 
from Luci.memory.semantic.store import _fts_query

class SqliteEpisodicStore:
    def __init__(self, conn: _sqlite3.Connection):
        self.conn = conn
        
    def add(self,summary:str,happended_at:str) -> None:
        self.conn.execute(
            "INSERT INTO episodes (happended_at, summary) VALUES (?,?)",
            (happended_at,summary)
        )
        self.conn.commit()
