...
Zero new dependencies — just stdlib urllib. Two backends:
  ...
  better   Tavily, if TAVILY_API_KEY (or LUCI_SEARCH_API_KEY) is set — an
...
def _tavily(query:str,key:str,max_results:int) -> list[tuple[str,str,str]]:
    body = json.dumps({"api_key":key,"query": query,"max_results":max_results,"include_answer":False}).encode()
...
def make_tool() -> Tool:
    def search_web(query: str, max_results: int = 5) -> str:
        key = os.getenv("TAVILY_API_KEY") or os.getenv("LUCI_SEARCH_API_KEY")
        try:
