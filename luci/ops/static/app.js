...
async function saveSettings(msgId){
  ...
  const toggles = {};
  document.querySelectorAll("[data-toggle]").forEach(i => { toggles[i.dataset.toggle] = i.checked ? "1" : ""; });
  const target = document.getElementById(msgId || "set-msg");
  if (target) target.textContent = "saving…";
  const r = await postJSON("/api/settings", {provider, model, keys, toggles});
  if (target) target.textContent = r.error ? ("Error: "+r.error) : (r.provider ? "Switched to "+r.provider+" — live now." : "Saved — live now.");
  if(!r.error) refresh();
}
...
function renderChatLog(){
  if (!CHAT.length)
    return `<div class="empty" style="padding:6px 2px">Message luci here from any tab. Open Overview to watch it flow through the harness, or the Gateway tab to see every channel's messages together.</div>`;
...
async function sendChat(fromInput){
  ...
  const pending = {role:"luci", pending:true, stream:""};
  CHAT.push(pending);
  syncChatLogs();
  try {
...
function archSVG(d){
  ...
  return `<div style="overflow-x:auto"><svg viewBox="0 0 1044 664" class="arch" role="img">
    ...
    ${box(32,72,128,56,"Gateway","cli · voice · web","gateway","","gateway")}
...
const DB_DESC = {
  ...
  applications: "job application tracker (Journal ▸ Applications)",
  learnings: "personal learning journal (Journal ▸ Learnings)",
  outreach_drafts: "drafted cold outreach messages (Journal ▸ Outreach)",
};
...
async function switchSession(id){
  ...
  if (r.ok){
    ...
    (r.history||[]).forEach(m => CHAT.push(m.role==="user"
      ? {role:"user", text:m.content} : {role:"luci", reply:m.content, historical:true}));
    syncChatLogs();
  }
  closeSessMenu();
}
...
function memOverview(d){
  ...
  return `<div class="card" style="border-color:var(--accent);background:var(--accent-soft)">
      ...
      <div class="r">This tab is the curated, per-pillar view of what luci remembers. The
      ...
      <code>.luci/state.db</code> — different altitude.
      <br><br>Some assistants (Hermes) keep memory as a single <code>MEMORY.md</code> file. luci keeps
...
function memSemantic(d){
  let h = `<div class="meta" style="margin-bottom:12px">Durable facts distilled from what you tell luci —
...
function memSkills(d){
  let h = `<div class="meta" style="margin-bottom:12px">Procedural memory — markdown instructions loaded
    only when a message matches. Add your own three ways: teach luci in chat (it calls
...
function memSoul(d){
  return `<div class="meta" style="margin-bottom:12px">SOUL.md is luci's persona — the system prompt it
    loads every turn. Editing it changes who your luci is. Changes are live next turn.</div>
...
function toolsMCP(t){
  ...
  let h = `<div class="card ${m.configured?"":""}" style="border-color:${m.live?"var(--good)":"var(--line2)"}">
    ...
    <div class="r">MCP lets luci borrow tools from any external server (files, GitHub, a database, …),
  ...
  h += `<h2>Connect one (30 seconds)</h2><div class="card">
    ...
    <div class="meta" style="margin-top:6px">2 — create ${reveal("","the .luci folder")}<code>/mcp.json</code>:</div>
  ...
  h += `<div class="meta" style="margin-top:12px">The same pattern scales: any MCP server (yours or a vendor's)
    plugs in the same way — no code changes to luci. Skills work the same way — drop a <code>SKILL.md</code>
    in ${reveal("skills","skills/")}.</div>`;
  return h;
}

// --- Journal: luci-specific tools whose output had nowhere to live before —
// applications, learnings, outreach drafts. Same subtab pattern as Memory/Database.
const APP_PILL = {offer:"pass", rejected:"fail", interviewing:"skip", applied:"skip", closed:""};
function journalApplications(d){
  const apps = d.applications || [];
  if (!apps.length) return `<div class="card empty">no applications tracked yet — track_application logs them here</div>`;
  return table(["company","role","status","when","notes"], apps.map(a => `<tr>
    <td>${esc(a.company)}</td><td class="meta">${esc(a.role)}</td>
    <td><span class="pill ${APP_PILL[a.status]||""}">${esc(a.status)}</span></td>
    <td class="meta">${esc((a.updated_at||a.applied_at||"").slice(0,10))}</td>
    <td class="meta">${esc(a.notes||"")}</td></tr>`));
}
function journalLearnings(d){
  const items = d.learnings || [];
  if (!items.length) return `<div class="card empty">nothing logged yet — log_learning writes here</div>`;
  return items.map(l => `<div class="card">
    <div class="u">${esc(l.topic)}${l.tags?` <span class="meta">· ${esc(l.tags)}</span>`:""}</div>
    <div class="r" style="margin-top:4px">${esc(l.content)}</div>
    <div class="meta" style="margin-top:6px">${esc((l.learned_at||"").slice(0,10))}${l.source?` · ${esc(l.source)}`:""}</div>
  </div>`).join("");
}
function journalOutreach(d){
  const items = d.outreach || [];
  if (!items.length) return `<div class="card empty">no drafts yet — draft_outreach writes here</div>`;
  return items.map(o => `<div class="card">
    <div class="u">${esc(o.company)}${o.contact?` · ${esc(o.contact)}`:""}</div>
    <div class="r" style="margin-top:4px">${renderMarkdown(o.draft||"")}</div>
    ${o.context?`<details style="margin-top:6px"><summary class="meta">context used</summary><pre>${esc(o.context)}</pre></details>`:""}
    <div class="meta" style="margin-top:6px">${esc((o.created_at||"").slice(0,10))}</div>
  </div>`).join("");
}
...
const VIEWS = {
  ...
  overview(d){
    ...
    ${archSVG(d)}
    <h2>Latest turn</h2>${d.turns.length?turnCard(d.turns[0]):'<div class="card empty">no turns yet — talk to luci first</div>'}`;
  },
  loop(d){
  ...
  settings(d){
    ...
    h += `<h2>Provider &amp; keys (BYOK)</h2><div class="card">
      ...
      <div style="margin-top:12px"><button class="save" onclick="saveSettings('set-msg')">Save &amp; switch</button>
    ...
    <h2>Web search key (optional)</h2><div class="card">
      ...
      <div style="margin-top:12px"><button class="save" onclick="saveSettings('search-msg')">Save</button>
        <span class="meta" id="search-msg" style="margin-left:10px">reads live — no restart needed for search</span></div>
    ...
    <h2>Calendar sync (optional)</h2><div class="card">
      <div class="meta" style="margin-bottom:10px">create_event always saves locally (state.db + calendar.ics).
        Turn either of these on to also write into your real calendar — both can be on at once.</div>
      <label class="fld chk"><input type="checkbox" data-toggle="LUCI_APPLE_CALENDAR" onchange="markEditing()" ${st.apple_calendar?"checked":""}>
        <span>Sync to Apple Calendar <span class="meta">(macOS only — first event asks Calendar.app for permission once)</span></span></label>
      <label class="fld chk"><input type="checkbox" data-toggle="LUCI_APPLE_TOOLS" onchange="markEditing()" ${st.apple_tools?"checked":""}>
        <span>Apple ecosystem tools <span class="meta">(Reminders, Notes, Mail — macOS only)</span></span></label>
      <label class="fld chk"><input type="checkbox" data-toggle="LUCI_GOOGLE_CALENDAR" onchange="markEditing()" ${st.google_calendar?"checked":""}>
        <span>Sync to Google Calendar <span class="meta">(any OS, via OAuth)</span></span></label>
      <div class="meta" style="margin:8px 0 4px">
        ${st.google_calendar
          ? (st.google_credentials_found
              ? `OAuth client file found at <code>${esc(st.google_credentials_path)}</code> — the first create_event call opens a browser to authorize once.`
              : `No OAuth client file at <code>${esc(st.google_credentials_path)}</code> yet. In Google Cloud Console: enable the Calendar API, create an OAuth client ID (type "Desktop app"), and save the downloaded JSON to that path.`)
          : `Off — flip the switch above, save, then create an event to trigger the one-time Google sign-in.`}
        Requires <code>pip install -e '.[google]'</code>.</div>
      <div style="margin-top:10px"><button class="save" onclick="saveSettings('cal-msg')">Save</button>
        <span class="meta" id="cal-msg" style="margin-left:10px"></span></div>
    </div>`;
    return h;
  },
  tools(d, sub){
    ...
    h += `<div class="meta" style="margin-bottom:12px">The capabilities the agent can call this turn.
      ...
      ${t.apple_on?"":"Apple tools are off (set <code>LUCI_APPLE_TOOLS=1</code>). "}Connect more via
      <a class="reveal" onclick="location.hash='tools/mcp'">MCP</a>.</div>`;
    h += `<h2>Calendar sync</h2>
      <div class="toolcard"><div class="tn">create_event / list_events
        <span class="srcpill ${t.apple_on?"":"apple"}">Apple ${t.apple_on?"on":"off"}</span>
        <span class="srcpill ${t.google_calendar_on?"":"apple"}">Google ${t.google_calendar_on?"on":"off"}</span></div>
      <div class="td">${t.google_calendar_on
        ? (t.google_calendar_ready
            ? (t.google_credentials_found
                ? "Google Calendar sync is on and ready — the first create_event call opens a browser to authorize once, then it's silent."
                : "Google Calendar sync is on but no OAuth client file was found yet — see Settings for setup steps.")
            : "Google Calendar sync is on but the extra isn't installed — run <code>pip install -e '.[google]'</code>.")
        : "Off. Every event still saves locally (state.db + calendar.ics). Turn on real-calendar sync in <a class=\"reveal\" onclick=\"location.hash='settings'\">Settings</a> or with <code>LUCI_APPLE_CALENDAR=1</code> / <code>LUCI_GOOGLE_CALENDAR=1</code>."}</div></div>`;
    ...
    if ((t.planned||[]).length){
      h += `<h2>Coming soon <span class="meta" style="font-weight:400">· on the architecture chart, not wired in yet (opt in with <code>LUCI_EXPERIMENTAL=1</code>)</span></h2>`;
  ...
  journal(d, sub){
    sub = sub || "applications";
    const tabs = [["applications","Applications",(d.applications||[]).length],
      ["learnings","Learnings",(d.learnings||[]).length],
      ["outreach","Outreach",(d.outreach||[]).length]];
    let h = `<div class="meta" style="margin-bottom:12px">What luci's own tools have written for you —
      the job search, what you've been learning, and drafted outreach. Talk to luci to add more:
      "track my application to Acme", "log that I learned X", "draft a message to Y".</div>`;
    h += subtabBar("journal", tabs, sub);
    if (sub === "learnings") return h + journalLearnings(d);
    if (sub === "outreach") return h + journalOutreach(d);
    return h + journalApplications(d);
  },
  database(d, sub){
    ...
    h += `<div class="card" style="border-color:var(--accent);background:var(--accent-soft)">
      ...
      uses a <code>MEMORY.md</code> file, luci uses these queryable tables — and mirrors them to a
    ...
    h += `<div class="card">
      ...
      <div class="meta">${kb} KB on disk · SQLite + FTS5 · open it yourself: <code>sqlite3 .luci/state.db</code></div>
      <div class="meta" style="margin-top:8px">${reveal("state.db","reveal state.db in Finder")} &nbsp;·&nbsp; ${reveal("","open the .luci folder")}</div></div>`;
  ...
  ops(d){
    ...
    h += `<div class="card"><span class="r">Every LLM call's tokens are logged to
      <code>.luci/usage.jsonl</code> (append-only, never wiped). Dollar cost is estimated from tokens
    ...
    h += (d.trace_tail||[]).length ? table(["event","detail","when"], d.trace_tail.map(e =>
      ...
      : `<div class="card empty">no trace lines yet — talk to luci</div>`;
...
const TITLES = {ops:"LLM Ops",
                database:"Database — everything luci stores (state.db)",
                journal:"Journal — applications, learnings, outreach"};
function render(){
  if (!D) return;
  ...
  document.getElementById("n-journal").textContent =
    (D.applications||[]).length + (D.learnings||[]).length + (D.outreach||[]).length;
...
const micHint = (msg) => { const i = document.getElementById("dmsg");
  if (i){ i.placeholder = msg; setTimeout(()=>{ i.placeholder = "Message luci…"; }, 8000); } };
