...
Where events land:
  ...
  opt-in      Apple Calendar, in a dedicated "luci" calendar, via AppleScript —
              set LUCI_APPLE_CALENDAR=1. First use makes macOS ask permission
              for your terminal to control Calendar; approve once.
  opt-in      Google Calendar (any OS), via OAuth — set LUCI_GOOGLE_CALENDAR=1.
              First use opens a browser for a one-time consent screen; see
              luci/tools/google_calendar.py for setup.

Apple and Google sync are independent switches — either, both, or neither can
be on. The tool's return string always says exactly where the event went — the
model relays it, so luci never over-claims what happened.
"""
...
def sync_to_apple_calendar(title: str, start: str, end: str, notes: str = "") -> str:
    """Create the event in Calendar.app under the 'luci' calendar (created on
    ...
    # Prefer a dedicated "luci" calendar, but macOS can't create calendars in
...
def make_tool(
    ...
    def create_event(title: str = "", start: str = "", end: str = "", attendees: str = "", notes: str = "") -> str:
        ...
        if google_calendar:
            from luci.tools.google_calendar import sync_to_google_calendar

            creds_path = google_credentials or (home / "google_credentials.json")
            where += " " + sync_to_google_calendar(home, creds_path, title, start, end, notes, attendees)
        if not apple_calendar and not google_calendar:
            where += (
                " Not synced to any calendar app (enable with LUCI_APPLE_CALENDAR=1 and/or "
                f"LUCI_GOOGLE_CALENDAR=1, or import manually: open {home / 'calendar.ics'})."
            )
        return (
...
def make_list_tool(conn: sqlite3.Connection) -> Tool:
    ...
    def list_events(start: str = "", end: str = "", limit: int = 20) -> str:
        ...
        if not rows:
            ...
            return f"No events found{window}. (This reads the local calendar in .luci/state.db.)"
